import { useMessages } from './useMessages';    // 메시지 관련 기능을 제공하는 Hook
import { useSearch } from './useSearch';        // 메시지 검색 관련 기능을 제공하는 Hook
import { useUpload } from './useUpload';        // 이미지 업로드 관련 기능을 제공하는 Hook
import { useModal } from './useModal';          // 모달(팝업창) 관리 관련 기능을 제공하는 Hook
import { useNavigate } from 'react-router-dom'; // 페이지 이동 기능을 제공하는 Hook
...
export const useChat = () => {
    ...
    const { 
        selectedImages: uploadImages,      // 현재 선택된 이미지 배열
        setSelectedImages: setUploadImages,  // 선택된 이미지 배열을 업데이트하는 함수
        handleImageUpload,                   // 이미지 업로드 처리 함수
        handleRemoveImage                    // 이미지 제거 함수
    } = useUpload();
    
    // useMessages Hook에서 채팅 메시지 관련 상태와 함수를 가져옴 (이미지 관련 상태 제외)
    const { 
    ...
    // 모달 관련 상태 및 함수를 useModal Hook에서 가져옴
    ...
    const inputProps = {
        ...
        selectedImages: uploadImages, // 현재 선택된 이미지 배열 (useUpload에서 가져옴)
        setSelectedImages: setUploadImages, // 선택된 이미지 배열 업데이트 함수
        handleImageUpload,          // 이미지 업로드 처리 함수
        handleRemoveImage           // 이미지 제거 함수
    };
    ...
    return {
        ...
        messageProps: {
            ...
            selectedImages: uploadImages,  // 업로드된 이미지 배열
            setSelectedImages: setUploadImages, // 이미지 배열 업데이트 함수
